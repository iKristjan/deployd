...
UserCollection.prototype.handle = function (ctx) {
  ...
  switch(ctx.req.method) {
    case 'GET':
      ...
      // Lower case usernames to prevent conflict.
      if (ctx.query && ctx.query.username) {
        ctx.query.username = ctx.query.username.toLowerCase();
      }
    ...
    case 'PUT':
      ...
      if (ctx.body.username) {
        ctx.body.username = ctx.body.username.toLowerCase();
      }
...
UserCollection.prototype.handleLogin = function (ctx) {
  ...
  // Lower case usernames to prevent conflicts.
